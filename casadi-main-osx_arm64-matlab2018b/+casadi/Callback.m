classdef  Callback < casadi.Function
  ...
  methods
    ...
    %
      if nargin==1 && strcmp(class(varargin{1}),'SwigRef')
        ...
        if strcmp(class(self),'director_basic.Callback')
          tmp = casadiMEX(984, 0, varargin{:});
        else
          tmp = casadiMEX(984, self, varargin{:});
        end
    ...
    function varargout = get_factory(self,varargin)
    %GET_FACTORY [INTERNAL] 
    %
    %  Function = GET_FACTORY(self, char name, {char} s_in, {char} s_out, struct:{char} aux, struct opts)
    %
